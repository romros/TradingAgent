#!/usr/bin/env python3
"""Compile the frozen GOOG D1 price-action discovery campaign."""
...
def compile_pilot(output_dir:Path):
 ...
 if source['ticker']!='GOOG' or source['end_exclusive']!='2025-01-01' or source['holdout_2025_accessed']:raise ValueError('source boundary mismatch')
 output_dir.mkdir(parents=True,exist_ok=True)
 registry={'markets':{'GOOG':{'research_eligible':True,'sq_symbol':s['sq_symbol'],'discovery_timeframe':'D1','discovery_slippage':0,'discovery_commission_per_order':0,'sq_resource_clone_from':'BTCUSD_ALQ_H4','sq_prune_resources':True,'sq_resource_remove_attributes':['cloneFrom','sourceTimezone'],'sq_resource_attributes':{'source':'1','barType':'1','precision':'D1','timezone':'America/New_York','dateFrom':epoch(p['train_from']),'dateTo':epoch(p['sealed_oos_to']),'uSymbol':'GOOG_IBKR_V1','uSymbolName':'GOOG_IBKR_V1','removeWeekends':'false','broker':'-1'},'sq_instrument_attributes':{'instrument':'GOOG_IBKR_V1','description':'GOOG adjusted D1 gross research','tickSize':'0.000001','tickStep':'0.000001','minDistance':'0','tickValueInMoney':'0','dateFrom':'0','dateTo':'0','rows':'0','totalDays':'0','defaultSpread':'0','defaultSlippage':'0','decimals':'6','commissions':'','pointValue':'1','dataType':'1','recognizedFromOrders':'false','exchange':'NASDAQ','country':'US','sector':'Communication Services','swap':'','orderSizeMultiplier':'1','orderSizeStep':'1','broker':'-1'},'exit_at_end_of_day':False,'eod_exit_seconds':None,'signal_time_range_seconds':None,'exit_at_end_of_range':False,'maximum_trades_per_day':1,'venue_max_leverage':1}}}
 ...
 cfx=output_dir/'project.cfx';d=s['discovery'];m=build(SCAFFOLD,cfx,'IBKR_GOOG_D1_PRICE_ACTION_V1','GOOG',rp,mp,date.fromisoformat(p['train_from']),date.fromisoformat(p['untouched_future_to']),d['accepted_limit'],d['search_profile'],d['generation'],d['attempt_budget'],d['wall_time_budget_minutes'],None,d['direction'],periods_override=periods)
...
if __name__=='__main__':print(json.dumps(compile_pilot(ROOT/'data/ibkr_sq_v2/goog_d1_capitulation_pilot'),indent=2))
