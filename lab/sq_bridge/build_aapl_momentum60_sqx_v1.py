...
def build(template:Path,output:Path)->dict:
 ...
 root=ET.fromstring(members['strategy_Portfolio.xml']);strategy=root.find('.//Strategy');strategy.set('name','AAPL_MOMENTUM60_MONTH_END_V1')
 ...
 if name is not None:name.text='AAPL_MOMENTUM60_MONTH_END_V1'
 settings.set('ResultName','AAPL_MOMENTUM60_MONTH_END_V1');members['settings.xml']=ET.tostring(settings,encoding='utf-8',xml_declaration=True)
 ...
 result={'schema_version':1,'strategy':'AAPL_MOMENTUM60_MONTH_END_V1','rule':'DayOfMonth[0] < DayOfMonth[1] AND Close[1] > Close[61]; enter market; exit after 20 bars','template_sha256':sha(template),'output_sha256':sha(output),'performance_accessed':False,'paper_authorized':False,'live_authorized':False};output.with_suffix('.receipt.json').write_text(json.dumps(result,indent=2)+'\n');return result
def main():
 a=argparse.ArgumentParser();a.add_argument('--template',type=Path,required=True);a.add_argument('--output',type=Path,required=True);z=a.parse_args();print(json.dumps(build(z.template,z.output),indent=2))
