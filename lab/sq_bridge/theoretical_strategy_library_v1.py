...
def build(spec_path=SPEC):
    ...
    for row in rows:
        for key,value in row.items():
            ...
            if hash_key not in row: raise ValueError(f"evidence hash absent: {row['id']}:{key}")
            path=ROOT/value
            if not path.is_file() or sha(path)!=row[hash_key]: raise ValueError(f"evidence mismatch: {row['id']}:{key}")
    admitted_statuses={'ADMITTED_RESEARCH_EDGE','ADMITTED_RESEARCH_EDGE_AND_INCREMENTAL_PORTFOLIO_COMPONENT','ADMITTED_CAPPED_PORTFOLIO_COMPONENT','ADMITTED_CAPPED_INCREMENTAL_PORTFOLIO_COMPONENT'}
    admitted=[r for r in rows if r['status'] in admitted_statuses];capped=[r for r in rows if r['status'] in {'ADMITTED_CAPPED_PORTFOLIO_COMPONENT','ADMITTED_CAPPED_INCREMENTAL_PORTFOLIO_COMPONENT'}];conditional=[r for r in rows if r['status'].startswith('CONDITIONAL_EDGE')];research_only=[r for r in rows if r['status']=='ADMITTED_RESEARCH_EDGE_NOT_ADMITTED_TO_PORTFOLIO'];rejected=[r for r in rows if r['status'].startswith('REJECTED')];watch=[r for r in rows if r['status'].startswith('WATCHLIST')]
