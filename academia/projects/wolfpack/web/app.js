...
async function refresh(){
  try {
    ...
    const ul=s.unified_ledger||{};
    document.querySelector('#monitor-paper').innerHTML=`Equity combinada${ul.cost_complete===false?' estimada':''}: <strong>${esc(fmt(ul.ending_equity_usdc??s.paper.ending_equity_usdc))} USDC</strong> · Wolfpack ${esc(fmt(ul.wolfpack_ending_equity_usdc??s.paper.wolfpack_ending_equity_usdc))} · setup independent ${esc(fmt(ul.standalone_net_pnl_usdc??s.paper.standalone_net_pnl_usdc))} USDC · ${esc(fmt(ul.open_count??s.coverage.paper_open))} obertes · ${esc(fmt(ul.closed_count??s.coverage.paper_closed))} tancades · cap ordre real autoritzada`;
