...
async function refresh(){
  try {
    ...
    document.querySelector('#monitor-paper').innerHTML=`Equity combinada${s.paper.combined_equity_is_estimate?' estimada':''}: <strong>${esc(fmt(s.paper.ending_equity_usdc))} USDC</strong> · Wolfpack ${esc(fmt(s.paper.wolfpack_ending_equity_usdc))} · setup independent ${esc(fmt(s.paper.standalone_net_pnl_usdc))} USDC · ${s.coverage.paper_open} obertes · ${s.coverage.paper_closed} tancades · cap ordre real autoritzada`;
