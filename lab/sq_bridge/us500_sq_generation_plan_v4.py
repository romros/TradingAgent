#!/usr/bin/env python3
"""Compile a screened US500 hypothesis into an auditable SQ generation plan."""
...
from lab.sq_bridge.us500_v4_hypotheses import (
    HYPOTHESIS_MARKET_SIDES, SEARCH_PROFILES, accepted_target,
)
...
def compile_plan(*, screen_path: Path, chain_path: Path,
    ...
    if (chain.get("market") != "US500"
            or chain.get("campaign_id") != "us500-d1-alquimia-v4"):
        raise ValueError("US500 generation chain identity mismatch")
    ...
    if (Path(receipts[1]["artifact"]).resolve() != screen_path
    ...
    profile, market_side = (SEARCH_PROFILES.get(hypothesis_id),
                            HYPOTHESIS_MARKET_SIDES.get(hypothesis_id))
    if profile is None or market_side is None:
        raise ValueError("screened US500 hypothesis has no translatable SQ profile")
    trace_path = _resolve(screen_path.parent,
                          screen.get("hypothesis_screen_trace_path", ""))
    ...
    period_contract_output.write_text(json.dumps(contract, indent=2,
                                                  sort_keys=True) + "\n")
    safe_id = re.sub(r"[^A-Za-z0-9]+", "_", hypothesis_id).strip("_").upper()
    project_name = f"ALQUIMIA_US500_D1_V4_{safe_id}"
    ...
    plan = {
        ...
        "source_hypothesis_id": hypothesis_id, "market": "US500",
        ...
        "periods": sq_periods(contract), "holdout_sealed": True,
        ...
        "alquimia_project_arguments": {
            "market": "US500", "name": project_name, "search_profile": profile,
