...
| 2026-08-11 | **Trace temporal derivat de l'export SQ**: `sq_temporal_trace_v4.py` transforma `orders.csv` + split canònic en trades train i finestres anuals validation/OOS, recalculant retorn long/short i durada. Rebutja holdout, trades entre segments i DST ambigu. `temporal_validation` reconstrueix el trace des de les fonts hashades i detecta edició del CSV/JSON. Pendent: Retest supervisat que lligui cada export al hash SQX concret; per tant encara no és una pipeline temporal completa. |
