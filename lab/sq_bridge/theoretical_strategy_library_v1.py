...
def build(spec_path=SPEC):
    ...
    admitted_statuses={'ADMITTED_RESEARCH_EDGE','ADMITTED_RESEARCH_EDGE_AND_INCREMENTAL_PORTFOLIO_COMPONENT'}
    admitted=[r for r in rows if r['status'] in admitted_statuses];conditional=[r for r in rows if r['status'].startswith('CONDITIONAL_EDGE')];research_only=[r for r in rows if r['status']=='ADMITTED_RESEARCH_EDGE_NOT_ADMITTED_TO_PORTFOLIO'];rejected=[r for r in rows if r['status'].startswith('REJECTED')];watch=[r for r in rows if r['status'].startswith('WATCHLIST')]
    ...
    return {'schema_version':1,'decision':'PORTFOLIO_LIBRARY_READY' if ready else 'BUILDING_LIBRARY','admitted_strategy_ids':[r['id'] for r in admitted],'conditional_strategy_ids':[r['id'] for r in conditional],'research_only_strategy_ids':[r['id'] for r in research_only],'watchlist_strategy_ids':[r['id'] for r in watch],'rejected_strategy_ids':[r['id'] for r in rejected],'admitted_count':len(admitted),'conditional_count':len(conditional),'eligible_portfolio_count':len(eligible),'distinct_eligible_mechanisms':distinct,'minimum_required':spec['minimum_portfolio_strategies'],'portfolio_ready':ready,'evidence_verified':True,'paper_authorized':False,'live_authorized':False}
