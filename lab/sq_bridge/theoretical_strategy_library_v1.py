...
def build(spec_path=SPEC):
    ...
    admitted=[r for r in rows if r['status']=='ADMITTED_RESEARCH_EDGE'];conditional=[r for r in rows if r['status']=='CONDITIONAL_EDGE'];rejected=[r for r in rows if r['status'].startswith('REJECTED')];watch=[r for r in rows if r['status'].startswith('WATCHLIST')]
    distinct=len({r['mechanism'] for r in admitted+conditional})
    ready=len(admitted)>=spec['minimum_portfolio_strategies'] and distinct>=spec['minimum_portfolio_strategies']
    return {'schema_version':1,'decision':'PORTFOLIO_LIBRARY_READY' if ready else 'BUILDING_LIBRARY','admitted_strategy_ids':[r['id'] for r in admitted],'conditional_strategy_ids':[r['id'] for r in conditional],'watchlist_strategy_ids':[r['id'] for r in watch],'rejected_strategy_ids':[r['id'] for r in rejected],'admitted_count':len(admitted),'conditional_count':len(conditional),'distinct_eligible_mechanisms':distinct,'minimum_required':spec['minimum_portfolio_strategies'],'portfolio_ready':ready,'evidence_verified':True,'paper_authorized':False,'live_authorized':False}
